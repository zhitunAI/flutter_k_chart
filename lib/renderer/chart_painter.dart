...
//    }
  ...
  void drawCrossLineText(Canvas canvas, Size size) {
    ...
    double y =
        selectY != 0 ? selectY : getMainY(point.close); //getMainY(point.close);
    double x;
    bool isLeft = false;
...
//For TrendLine
  ...
  void drawCrossLine(Canvas canvas, Size size) {
    ...
    double y = selectY != 0
        ? selectY
        : getMainY(point.close); //getMainY(point.close); //跟着价格走的
    ...
    if (scaleX >= 1) {
      canvas.drawOval(
          Rect.fromCenter(
              center: Offset(x, y), height: 4.0 * scaleX, width: 4.0),
          paintX);
    } else {
      canvas.drawOval(
          Rect.fromCenter(
              center: Offset(x, y), height: 4.0, width: 4.0 / scaleX),
          paintX);
    }
  }
